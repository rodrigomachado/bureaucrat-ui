export class EntityMeta {
  ...
  validateEntity(entity: Entity): Entity {
    ...
    if (dataFieldsCount !== metaFieldsCount) throw new Error(
      `Fields length does not match: ${metaFieldsCount} fields on metadata for ${this.name},` +
      ` ${dataFieldsCount} fields on providade data`
    )
    for (const fieldCode in this.fields) {
      const meta = this.fields[fieldCode]
      if (!meta) throw new Error(`Field '${fieldCode}' not found in entity '${this.code}'`)
    }
    return entity
  }

  keyFor(entity: Entity): string {
    const ids = Object.values(this.fields).filter(x => x.identifier).map(x => entity[x.code])
    if (!ids.length) throw new Error(`EntityType "${this.name}" has no identifiers`)
...
export function formatTitlePattern(type: EntityMeta, data: { [name: string]: any }, pattern: string): string {
  ...
  while (match = /#\{([^}]+)\}/.exec(formatted)) {
    ...
    if (!type.fields[fieldName]) throw new Error(
      ...
      `Field '${fieldName}' mentioned in formatting pattern not found. Pattern: '${pattern}'`
    )
